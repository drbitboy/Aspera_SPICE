...
from ramangle.ramangle_algorithm import ramangle_instr
...
def GeoLoop(t_initial, t_final, step,file_name):
    ...
    header = ['UTC','Altitude (km)','Cartesian Position wrt Earth (J200, km)','Cartesian Position wrt Sun (J200, km)','Beta Angle (deg)', 'Beta0 (deg)',
              ...
              'Limb Angle (slit1) (deg)','Limb Angle (slit2) (deg)', 'Moon Boresight (slit1) (deg)','Moon Boresight (slit2) (deg)',
              'Sun Boresight (slit1) (deg)','Sun Boresight (slit2) (deg)', 'Ram Angle (slit1) (deg)','Ram Angle (slit2) (deg)', 
              'BSC difference (ASP bod and ASP_SLIT1)', 'BSC difference (ASP body and ASP_SLIT2)','FOV (slit1 visiblity, slit2 visiblity)', 
              'OBSGEO', 'vel_earth_los', 'vel_sc_los (slit1)', 'vel_sc_los (slit2)', 'slitpa (slit1)', 'slitpa (slit2)']
    data = []
    while res_date <= end:
        ...
        sun_boresight1 = sunboresight_instr(UTC, Target, instr1)
        sun_boresight2 = sunboresight_instr(UTC, Target, instr2)
        ram_angle1 = ramangle_instr(UTC, Target, instr1)
        ram_angle2 = ramangle_instr(UTC, Target, instr2)
        asp_slit1 = boresight_for_difference(UTC, Target, galaxy_targ, instr1)
        asp_slit2 = boresight_for_difference(UTC, Target, galaxy_targ, instr2)
        visible = get_fov(UTC, galaxy_targ)
        obs= obsgeo(UTC, Target, planetEarth)
        vel_earth= vel_earth_los(UTC, galaxy_targ)
        vel_sc_los1 = vel_sc_los(UTC, Target, galaxy_targ, instr1)
        vel_sc_los2 = vel_sc_los(UTC, Target, galaxy_targ, instr2)
        ...
        csv_line = [UTC,alt,ptarg1,ptarg2,betadeg,beta_0,eclipsed_Sun, eclipsed_Moon,lon,lat,ra,dec,
                    ...
                    ram_angle1,ram_angle2, asp_slit1, asp_slit2, visible, obs, vel_earth, vel_sc_los1, 
